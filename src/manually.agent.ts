...
const getCurrentWeather = (param?: any) => {
  const weather = {
    temperature: "2",
    unit: "F",
    forecast: "snowy",
...
async function agent(query: string) {
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    if (foundActionStr) {
      ...
      const [_, action, actionArg] = actions as RegExpExecArray;
      ...
      const observation = await availableFunctions[
        action as AvailableFunctionNames
      ](actionArg);
      messages.push({
        role: "assistant",
